...
func NewProfileServH[T models.IntUsr](genServ *providers.IProviderS, vSvc core_interfaces.IVerificationService, sesSvc core_interfaces.ISessionService, accountRepo repoimpl.IOAuthAccountRepo) *Service[T] {
	return &Service[T]{
		ProvServ:    genServ,
		VSvc:        vSvc,
		SesSvc:      sesSvc,
...
func (aus *Service[T]) ChangePassword(ctx context.Context, userId, sessionId string, input models.PasswordUpdateDto) (dtos.GResp[T], error) {
	...
	account, err := aus.accountRepo.GetByUserAndProvider(ctx, userId, models.ProvCredential)
	...
	err = aus.TxMgr.Transaction(ctx, func(txCtx context.Context) error {
		_, err := aus.accountRepo.Update(txCtx, account.ID, map[string]interface{}{"password": hash})
		if err != nil {
...
func (aus *Service[T]) SendChangeEmail(ctx context.Context, userId string, input models.ChangeEmailReqDto) (dtos.GResp[bool], error) {
	...
	return aus.VSvc.SendVerification(ctx, input.NewEmail, models.PurposeChangeEmail, &core_interfaces.VerOpt{UserId: resp.Body.GetID()})
}
...
func (aus *Service[T]) VerifyChangeEmail(ctx context.Context, userId string, input models.VerifyEmailDto) (dtos.GResp[bool], error) {
	...
	err = aus.TxMgr.Transaction(ctx, func(txCtx context.Context) error {
		...
		updateResp, err := sql_db.DbUpdateOneById[T](gormutil.GetDB(txCtx, aus.ProvServ.GormConn), txCtx, userId, models.UserDto{Email: &newEmail, EmailVerified: true}, nil)
		if err != nil {
			return err
		}
		...
		account, err := aus.accountRepo.GetByUserAndProvider(txCtx, userId, models.ProvCredential)
		if err == nil && account != nil {
			_, err = aus.accountRepo.Update(txCtx, account.ID, map[string]any{"account_id": newEmail})
			if err != nil {
